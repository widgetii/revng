...
let
  getVS17Files =
    ...
    let
      trace = message: result: builtins.trace (message result) result;
      simpleTrace = value: builtins.trace value value;
      size = list: builtins.toString (builtins.length list);
      ...
      englishOnlyPackages = trace (packages: "${size packages} are in English") (
        builtins.filter (
          entry: !(pkgs.lib.hasAttr "language" entry) || (pkgs.lib.toLower entry.language) == "en-us"
        ) allPackages
      );
      selectedLower = builtins.map (package: pkgs.lib.toLower package) selected;
      roots = trace (roots: "There are ${size roots} roots") (
        builtins.filter (entry: builtins.elem (pkgs.lib.toLower entry.id) selectedLower) englishOnlyPackages
      );
      englishOnlyPackagesAttrs = pkgs.lib.listToAttrs (
        builtins.map (package: {
          name = pkgs.lib.toLower package.id;
          value = package;
        }) englishOnlyPackages
      );
      parseDependencies =
        ...
        builtins.map (
          dep:
          pkgs.lib.throwIf (
            builtins.typeOf dep.value == "set" && (pkgs.lib.hasAttr "id" dep.value)
          ) "Unexpected value: ${builtins.toJSON dep.value} for ${dep.name}" (pkgs.lib.toLower dep.name)
        ) (pkgs.lib.attrsToList deps);
      findPackage =
        name:
        if builtins.hasAttr name englishOnlyPackagesAttrs then
          [ englishOnlyPackagesAttrs.${name} ]
        else
          builtins.trace "Could not find ${name}" [ ];
      findPackageNew =
        name: builtins.filter (package: pkgs.lib.toLower package.id == name) englishOnlyPackages;
      recursiveDependencies =
        package: stack:
        builtins.trace
          "${pkgs.lib.concatStrings (pkgs.lib.replicate (builtins.length stack) "  ")}${package.id}"
          (
            if builtins.elem package.id stack then
              [ ]
            else
              (
                [
                  (pkgs.lib.toLower package.id)
                ]
                ++ (
                  if !(pkgs.lib.hasAttr "dependencies" package) then
                    [ ]
                  else
                    (pkgs.lib.flatten (
                      builtins.map (
                        dependency:
                        builtins.map (entry: recursiveDependencies entry (stack ++ [ package.id ])) (findPackage dependency)
                      ) (parseDependencies package.dependencies)
                    ))

                )
              )
          );
      requiredPackages = trace (list: "We are interested in ${size list} packages") (
        pkgs.lib.unique (pkgs.lib.flatten (builtins.map (entry: recursiveDependencies entry [ ]) roots))
      );
      requiredFiles = trace (list: "We are interested in ${size list} files") (
        pkgs.lib.flatten (
          builtins.map (
            packageName:
            builtins.map (
              package:
              if pkgs.lib.hasAttr "payloads" package then
                (builtins.map (payload: {
                  id = package.id;
                  filename = pkgs.lib.replaceString "\\" "/" payload.fileName;
    ...
    in
    builtins.toFile "packages.json" (builtins.toJSON requiredFiles);
  vs17Files = getVS17Files {
    selected = [
    ...
    url = "https://download.visualstudio.microsoft.com/download/pr/5011a9cc-e8ef-42cb-ad72-87de1031accc/b674dd46f83a684142689b3a86c73ba5f2e2477018087e4df218516214bbad58/VisualStudio.vsman";
    hash = "sha256-Ajqdff1SGjWrYMEwWAIrao1Al1gKEFV1A7jqPtk/2i4=";
  };
  wineWrapper =
    ...
    in
    {
      ...
      extract =
        let
          ...
          }) (builtins.fromJSON (builtins.readFile jsonIndex));
          by_extension =
            what: extension: builtins.filter (entry: pkgs.lib.hasSuffix extension entry.name) what;
          vsix = by_extension downloads ".vsix";
          sdk = builtins.filter (
            entry: entry.component == sdkComponent && pkgs.lib.hasPrefix "Installers/" entry.name
          ) downloads;
          sdk_cab = by_extension sdk ".cab";
          sdk_msi = by_extension sdk ".msi";
          paths =
            entries: "'" + (pkgs.lib.concatStringsSep "' '" (builtins.map (entry: entry.path) entries)) + "'";
        in
        ''
          log "Extracting all the *.vsix files"
          mkdir extracted-vsix
          cd extracted-vsix
          for VSIX_FILE in ${paths vsix}; do
...
[
  ...
  # 32-bit host, 32-bit target
  (msvcPackager (vs17Configuration "i386" vs17Files))
